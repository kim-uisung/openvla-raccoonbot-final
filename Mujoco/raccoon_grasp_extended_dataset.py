...
INSTRUCTION_TEMPLATES = {
    "cylinder": [
        ...
        "take the {color} cylinder",
        "get the {color} cylinder",
    ],
    "sphere": [
        "grasp the {color} sphere",
        "pick up the {color} sphere",
        "grab the {color} sphere",
        "take the {color} sphere",
        "get the {color} sphere",
    ],
}
...
class DatasetLogger:
    ...
    def start_episode(self, episode_id, instruction, goal_xy, box_init_xy,
                      box_init_yaw, task_type="grasp", target_color=None,
...
class ExtendedSyncSimRaccoon:
    ...
    def is_grasp_success(self, target_body_name, touch_threshold=0.1):
        try:
        ...
        if not (both_touched and gripper_closing):
            return False

        target_body_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, target_body_name)
        all_object_ids = set()
        for bmap in [CYLINDER_BODY_MAP, SPHERE_BODY_MAP]:
            for bname in bmap.values():
                bid = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, bname)
                if bid != -1:
                    all_object_ids.add(bid)
        ...
        for contact_idx in range(int(self.data.ncon)):
            ...
            other_geom = contact.geom2 if body1 == target_body_id else contact.geom1
            other_geom_name = mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_GEOM, int(other_geom)) or ""
            if other_geom_name == "floor":
                continue
            if other_body in all_object_ids:
                continue
            return True
        return False
...
def run_episode(rc, logger, episode_id, instruction, cylinder_specs, sphere_specs,
                ...
                initial_settle=0.3, hz=10, touch_threshold=0.1):
    ...
    logger.start_episode(
        episode_id=episode_id, instruction=instruction, task_type="grasp",
    ...
    try:
        ...
        success = rc.is_grasp_success(target_body_name, touch_threshold)
...
def collect_dataset(
    ...
    dataset_root="raccoon_grasp_extended",
    num_episodes=300,
    keep_failed=False,
    use_viewer=False,
    ...
    # cylinder 4색 + sphere 4색 = 8종류 균등 분배
    ...
    rc = ExtendedSyncSimRaccoon(xml_path=xml_path, image_size=(256, 256),
        ...
        while sum(success_counts.values()) < num_episodes and attempt_count < max_attempts:
            ...
            try:
                ...
                success = run_episode(
                    ...
                    hz=hz, touch_threshold=touch_threshold,
                )
...
if __name__ == "__main__":
    collect_dataset(
        ...
        dataset_root="/data/Raccoonbot_Openvla/Mujoco/raccoon_grasp_extended",
        num_episodes=300,
        keep_failed=False,
        use_viewer=False,
        camera_name="front_view",
